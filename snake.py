...
tela = pygame.display.set_mode(dims)
...
def desenha_cobra(lista_cobra):
    ...
    for unidade in lista_cobra:
        pygame.draw.rect(tela, branco, [unidade[0],unidade[1],d,d])
...
def verifica_comida(dx,dy,x_comida,y_comida,lista_cobra,v):
    ...
    return x_comida, y_comida, lista_cobra, v
...
def atualizar_pontos(lista_cobra):
    ...
    tela.blit(escore, [5, 5])

while True:
    pygame.display.update()
    desenha_cobra(lista_cobra)
    dx, dy, lista_cobra = mover_cobra(dx, dy, lista_cobra)
    x_comida, y_comida, lista_cobra, v = verifica_comida(dx, dy, x_comida, y_comida, lista_cobra, v)
